...
@router.get("/", response_model=List[schemas.ProyectoOut], summary="Listar proyectos (filtro: estado, presupuesto)")
...
@router.delete("/{proyecto_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Eliminar proyecto (en cascada)")
def eliminar_proyecto(proyecto_id: int, db: Session = Depends(get_db)):
    crud.eliminar_proyecto(db, proyecto_id)
    return None


@router.post("/{proyecto_id}/asignar", summary="Asignar empleado a proyecto")
...
@router.delete("/{proyecto_id}/desasignar/{empleado_id}", summary="Desasignar empleado de proyecto")
